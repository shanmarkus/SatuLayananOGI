...
var ContentHTTPClient = Titanium.Network.createHTTPClient({
    onload : function(e) {
        ...
        var webview = Ti.UI.createWebView({
            ...
            html : "<head><meta name=\"viewport\" content=\"width=device-width, user-scalable=no\"></head><style>" + "a.button-link {color: white; width: 81px; height: 23px; float: left; display: block; background: url('http://satulayanan.net/img/btn-link.png'); margin-right: 5px;}" + "body{font-family:'HelveticaNeue-Light';font-size:15px}" + "img{max-width:300px;} " + ".content{padding:5px}" + "td, th{ width: 4rem;height: 2rem;border: 1px solid #ccc;text-align: center;}" + "th {background: lightblue;border-color: white;}" + "body {padding: 1rem;}</style><div class='content'>" + pengelolaHTML + regex(jsonObject) + "<div style='width:100%'>" + discusPage + "</div>" + footer + "</div>",
            top : 5,
            height : 480,
