...
detailModuleTable.addEventListener('scroll', function(e){
	...
	if(pulling && !reloading && e.contentOffset.y <= -80.0)
	{
		...
		detailModuleTable.setContentInsets({top:80},{animated:true});
...
detailModulesHTTPClient.onload = function() {
	...
	if(reloading == true){
		...
		detailModuleTable.setContentInsets({top:0},{animated:true});
		reloading = false;
		statusLabel.text = "Pull to refresh...";
		actIndicator.hide();
		arrowImage.backgroundImage = 'img/refreshArrow.png';
...
detailModulesHTTPClient.onerror = function() {
