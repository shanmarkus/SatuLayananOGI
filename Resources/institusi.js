...
var isLoad=false;
var institusiHTTPClient = Titanium.Network.createHTTPClient({
    onload : function(e) {
        ...
        institusiTable.data = data;

        if (reloading == true) {
            //when done, reset the header to its original style
            institusiTable.setContentInsets({
                top : 0
            }, {
                animated : true
            });
...
//create a table view
var institusiTable = Titanium.UI.createTableView({
...
institusiTable.headerPullView = tableHeader;
win.add(institusiTable);
...
institusiTable.addEventListener('scroll', function(e) {
...
institusiTable.addEventListener('dragEnd', function(e) {
    if (Ti.Platform.osname != 'iphone') {
        return;
    }
    // offset = e.contentOffset.y;
    if (pulling && !reloading && offset <= -65.0) {
        ...
        //null out the existing module data
         if(!isLoad)institusiTable.data = null;
        data = [];
...
function loadInstitusi() {
    //open the recipes xml feed
    var bool = institusiHTTPClient.open('GET', 'http://satulayanan.net/api/index?tag=get_user_role&id=2');
    //execute the call to the remote feed
    institusiHTTPClient.send();
